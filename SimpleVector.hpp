...
public:
	 ...
	 */
	inline SimpleVector(int size);
	//! Destructor
	virtual ~SimpleVector();

	//! Set all the entry of the Vector equal to val
	inline SimpleVector &  operator=(const double & val);
	//! Set the entry of the Vector equal to the entries in RHS
	inline SimpleVector & operator=(const SimpleVector & RHS);
	...
	inline const double at(const int i) const;
	...
	//! result = v1 + c2*v2
	inline friend void add(const SimpleVector & v1, const double & c2, const SimpleVector & v2, SimpleVector & result);
	//! result = c1*v1 + c2*v2
	inline friend void add(const double & c1, const SimpleVector & v1, const double & c2, const SimpleVector & v2, SimpleVector & result);
	//! result = alpha(v1 + v2)
	inline friend void add(const double & alpha, const SimpleVector & v1, const SimpleVector & v2, SimpleVector & result);
	//! result = v1 + v2 + v3
	inline friend void add(const SimpleVector & v1, const SimpleVector & v2, const SimpleVector & v3, SimpleVector & result);
	//! result = v1 - v2
	inline friend void subtract(const SimpleVector & v1, const SimpleVector & v2, SimpleVector & result);
	//! return the inner product of v1 and v2
	inline friend double InnerProduct(const SimpleVector & v1, const SimpleVector & v2);

...
int main(int argc, char** argv){
  ...
  if(impulseIn["impulseDirection"].size() == 3){
	std::cout << "impulse direction provided" << std::endl;
	impulseInfo.initialImpulse =
	  Eigen::Vector3d(
		  impulseIn["impulseDirection"][0].asDouble(),
		  impulseIn["impulseDirection"][1].asDouble(),
		  impulseIn["impulseDirection"][2].asDouble()).normalized();
	std::cout << "initial impulse: " << impulseInfo.initialImpulse << std::endl;
  } else {
...
void drawArrow(const Eigen::Vector3d& direction, const Eigen::Vector3d& pointPosition){
  ...
  double length = 10*(direction.norm()/impulseInfo.maxScale);
...
const double nSpiralTimesteps = 0;

Eigen::Vector3d getVectorForFrame(const Eigen::Vector3d v, int timestep){


  return impulseInfo.maxScale*std::min(static_cast<double>(timestep)/nScaleTimesteps, 1.0)*v;
...
<float name="alpha" value="0.3"/>
  ...
  */
  ...
  outs << "<shape type=\"ply\" > \n"
	   ...
	   << meshFilename << "\" />\n"
	   << "<bsdf type=\"blendbsdf\">\n"
	   << "<texture name=\"weight\" type=\"wireframe\" />\n"
	   << "<bsdf type=\"twosided\" ><bsdf type=\"diffuse\"><spectrum name=\"reflectance\" value=\"0\" /></bsdf></bsdf>\n"
	   << "<bsdf type=\"twosided\">\n"
	   << "<bsdf type=\"diffuse\">\n"

...
namespace igl{
  ...
  static bool list_to_matrix(const std::vector<std::vector<T > > & V,Mat & M)
  {
	// number of rows
	int m = V.size();
	if(m == 0){
	  M.resize(0,Eigen::NoChange);
	  return true;
	}
	// number of columns
	auto firstRowSize = V.front().size();
	if(!std::all_of(V.begin(), V.end(),
			[&firstRowSize](const std::vector<T>& row){
  ...
  template <typename T, class Mat>
  static bool list_to_matrix(const std::vector<T > & V,Mat & M)
  {
	// number of rows
	int m = V.size();
	if(m == 0){
